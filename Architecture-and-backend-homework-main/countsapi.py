...
def countsapi2(tid,sdate,edatestr):
    ...
    feesarr = [["Nationals",13],["Mets",13.5],["Reds",11],["Angels",18],["Pirates",18],["Marlins",10],["Rays",11],["Phillies",22],["RedSox",32],["PhilliesSpringTraining",6.5]]
    # tid = "12"
    # sdate = "12232022"
    # edatestr = "2023-06-06"
    print(tid)
    ...
    if team != "":
        ...
        if ed_count > 1:
            for data in results:
                ...
                datadf["COUNT"] = datadf.apply(lambda x: x["COUNT_SUM"] if x["CODE"] in ["GA","LAWNS","LAWN","BERM"] else x["COUNT"],axis=1)
                datadf["CAPACITY"] = datadf.apply(lambda x: x["CAPACITY_SUM"] if x["CODE"] in ["GA","LAWNS","LAWN","BERM"] else x["CAPACITY"],axis=1)
                ...
                datadf = datadf.merge(countgroupdf,how="left",suffixes=(None,"_rank"),left_index=True,right_index=True)
                datadf = datadf.merge(pricegroupdf,how="left",suffixes=(None,"_rank"),left_index=True,right_index=True)
                ...
                broadresults = requests.get("https://api.blabla.com/event_listings.php?token=blabla&eventid=%s&includeTags=zone"%(skyid))
                ...
                if broaddf.empty:
                    ...
                    finalcolstokeep = ["section","CODE","EVENT","DATE","BROADCAST","newbroad","BG","bgnotes","FP","fpnotes","tagstoadd","tagstoremove","COUNT_SUM","THRESHOLD","CAPACITY","PRICE",
                    ...
                    broaddf = broaddf.loc[broaddf["notes"].str.contains(".com") == False]
                    ...
                    cleandf[["bgnotes","fpnotes","newbroad"]] = cleandf.apply(lambda x: notes(x,teamname),axis=1,result_type="expand")
                    # cleandf.to_csv("AfternotesCleandf.csv",index=False)
                    cleandf[["tagstoadd","tagstoremove"]] = cleandf.apply(lambda x: tags(x),axis=1,result_type="expand")
                    finalcolstokeep = ["section","CODE","EVENT","DATE","BROADCAST","newbroad","BG","bgnotes","FP","fpnotes","tagstoadd","tagstoremove","COUNT_SUM","THRESHOLD","CAPACITY","PRICE",
                    "COUNT","PUBLIC DESC","DESC"]
                    ...
                    if broaddf.empty:
                        ...
                        for f in feesarr:
                            ...
                            if teamname in t:
                                ...
                                fees = 10
                        cleandf["fees"] = fees
                        groubbydf = pd.crosstab(cleandf["CODE"],cleandf["BROADCAST"],dropna=False).reset_index(names="CODE")
                        print(groubbydf)
                        cleandf = cleandf.merge(groubbydf,on="CODE",how="left")
        ...
        cleandf[["section","CODE","EVENT","DATE","BROADCAST","newbroad","BG","bgnotes","FP","fpnotes","tagstoadd","tagstoremove","COUNT_SUM","THRESHOLD","CAPACITY","PRICE",
                    "COUNT","PUBLIC DESC","DESC"]] = "No Tickets Data"
